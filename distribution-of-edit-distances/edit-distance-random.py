...
bio_dna_seq = "".join(file_lines)
bio_dna_seq = bio_dna_seq.replace("\n", "")
input_file.close()

# creating a random sequence with equal to the length of MTB dna
random.seed("0")
dna_seq = ''.join(random.choice('ACGT') for base in range(len(bio_dna_seq)))
